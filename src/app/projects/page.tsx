...
export default async function Page({ searchParams }: { searchParams?: Promise<{ tag?: string; q?: string; sort?: string; view?: string }> }) {
  ...
  return (
    <div className="w-full py-10">
      <h1 className="text-2xl font-semibold">Projects{tag ? ` · #${tag}` : ""}</h1>
      <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-400">Case studies and shipped work across comms, AI systems, and product.</p>
      <FilterBar allTags={allTags} placeholder="Search projects..." />
      <FeaturedStrip items={featured} base="/projects" />
      <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((d) => (
          <ContentCard key={d.slug} doc={d} href={`/projects/${d.slug}`} tagBase="/projects" variant={view === "compact" ? "compact" : "default"} />
        ))}
      </div>
    </div>
  );
