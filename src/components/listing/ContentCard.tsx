...
export default function ContentCard({ doc, href, tagBase, variant = "default", hardLink = false }: Props) {
  ...
  return (
    ...
    >
      ...
      {hardLink ? (
        <a href={href} className={variant === "compact" ? "text-[13px] font-semibold text-neutral-900 hover:underline dark:text-neutral-100" : "text-sm font-semibold text-neutral-900 hover:underline dark:text-neutral-100"}>{frontmatter.title}</a>
      ) : (
